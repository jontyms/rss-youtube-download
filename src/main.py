...
os.system("youtube-dl -i --dateafter now-1week -a vidoes.txt --download-archive ~/.youtube --add-metadata --write-all-thumbnails --embed-thumbnail --write-info-json --embed-subs --all-subs -o '~/youtube/%(uploader)s/%(title)s.%(ext)s' -f 'bestvideo[height<=1080]+bestaudio/best[height<=1080]'")
time.sleep(3*60)